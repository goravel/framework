...
func TestMapCollectFunction(t *testing.T) {
	...
	assert.Equal(t, []string{"item-0-1", "item-1-2", "item-2-3", "item-3-4", "item-4-5"}, numberStrings.All())
	...
	people := Of([]Person{Person{Name: "Alice", Age: 25}, Person{Name: "Bob", Age: 17}, Person{Name: "Charlie", Age: 30}})
	...
	for i, summary := range summaries.All() {
		...
		assert.Equal(t, expected.DisplayName, summary.DisplayName)
		assert.Equal(t, expected.IsAdult, summary.IsAdult)
	}
	...
	assert.Equal(t, []int{6, 8, 10}, result.All())
}
...
func TestGenericReduceFunction(t *testing.T) {
	...
	words := Of([]string{"Hello", "World", "Test"})
	...
	assert.Equal(t, expectedStats.Total, ageStats.Total)
	assert.Equal(t, expectedStats.Count, ageStats.Count)

	avgAge := float64(ageStats.Total) / float64(ageStats.Count)
	assert.Equal(t, 24.0, avgAge)
	...
	assert.Equal(t, 100, emptyResult)
	...
	// 0*1 + 1*2 + 2*3 + 3*4 + 4*5 = 40
	assert.Equal(t, 40, indexProduct)
...
func TestReplace(t *testing.T) {
	...
	assert.Equal(t, []int{10, 2, 30, 4, 5}, result.All()) // index 10 is out of bounds, skipped
	assert.Equal(t, []int{1, 2, 3, 4, 5}, c.All())
}
