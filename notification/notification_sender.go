...
// NotificationSender coordinates sending notifications via registered channels.
// It supports both immediate dispatch (SendNow) and queued dispatch (Send).
...
// NewNotificationSender creates a new NotificationSender with the provided facades.
...
// Channels are resolved per notifiable using Notif.Via and configured with facades.
...
// queueNotification serializes the notifiable and notification, then dispatches
// a SendNotificationJob with the target channels for asynchronous processing.
