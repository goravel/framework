...
func (s *VendorPublishCommandTestSuite) TestPublish() {
	...
	s.False(command.publish("test.go", "123", true, false))
	s.True(command.publish("test.go", "123", false, false))
	s.True(file.Contain("test.go", "123"))
	s.False(command.publish("test.go", "123", false, false))
	s.True(command.publish("test.go", "111", false, true))
	s.True(file.Contain("test.go", "111"))
	s.True(command.publish("test.go", "222", true, false))
	s.True(file.Contain("test.go", "222"))
	s.True(command.publish("test.go", "333", true, true))
