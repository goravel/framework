...
func main() {
	config, err := file.GetPackageContent(packages.GetModulePath(), "setup/config/DummyName.go")
	if err != nil {
		panic(err)
	}
	...
	packages.Setup(os.Args).
		Install(
			...
			modify.File(path.Config("DummyName.go")).Overwrite(config),
		).
		Uninstall(
			modify.GoFile(path.Config("app.go")).
				Find(match.Providers()).Modify(modify.Unregister("&DummyName.ServiceProvider{}")).
				Find(match.Imports()).Modify(modify.RemoveImport(packages.GetModulePath())),
			modify.File(path.Config("DummyName.go")).Remove(),
		).
		Execute()
}
