...
var (
	...
	AIFailoverInsufficientCredits         = New("ai: provider %s has insufficient credits")
	AIFailoverPatternInvalid              = New("ai: provider %s failover reason %s has invalid regex %s: %v")
	AIFailoverProviderOverloaded          = New("ai: provider %s is overloaded")
	AIFailoverRateLimited                 = New("ai: provider %s was rate limited")
	AIFailoverReason                      = New("ai: provider %s failed over because %s")
	...
	EventHandlePanic             = New("event %s panicked while preparing its payload: %v")
	EventInvalidEvent            = New("invalid event: %v")
	EventInvalidListener         = New("invalid listener %s, expect event.Listener, func(event any, args ...any) error or func(event *SomeEvent) error")
	EventListenerEmptySignature  = New("listener %s must have a non-empty signature")
	...
	EventListenerPanic           = New("listener of event %s panicked: %v")
	EventListenerTypeMismatch    = New("listener of event %s expects %s, got %T")
	EventQueueDuplicateSignature = New("signature %s is already registered by another listener")
	EventQueueMissingEvent       = New("queued listener %s received no event")
	EventTooManyPayloads         = New("event %s received %d payloads, at most one is accepted")
	...
	NotificationChannelNotFound               = New("notification channel not found: %s").SetModule(ModuleNotification)
	...
	NotificationMailEmptyRoute                = New("mail channel: %T returned an empty address").SetModule(ModuleNotification)
	...
	NotificationDatabaseEmptyRoute            = New("database channel: %T returned an empty ID").SetModule(ModuleNotification)
	...
	NotificationQueuePayloadDecodeFailed      = New("notification queue payload could not be decoded: %v").SetModule(ModuleNotification)
	...
	PackageGoModDownloadFailed   = New("failed to download go.mod dependencies: %s")
