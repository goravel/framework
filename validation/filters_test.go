...
func TestExpandWildcardFields(t *testing.T) {
	t.Run("no wildcards", func(t *testing.T) {
		result := expandWildcardFields(map[string][]ParsedRule{
			"name":  {{Name: "trim"}},
			"email": {{Name: "lower"}},
		}, []string{"name", "email"}, false)

		assert.Len(t, result, 2)
		assert.Equal(t, "trim", result["name"][0].Name)
		assert.Equal(t, "lower", result["email"][0].Name)
	})

	t.Run("wildcard expansion", func(t *testing.T) {
		result := expandWildcardFields(map[string][]ParsedRule{
			"users.*.name": {{Name: "trim"}},
		}, []string{"users", "users.0", "users.0.name", "users.0.email", "users.1", "users.1.name", "users.1.email"}, false)

		assert.Len(t, result, 2)
		assert.Equal(t, "trim", result["users.0.name"][0].Name)
		assert.Equal(t, "trim", result["users.1.name"][0].Name)
	})

	t.Run("keepUnmatched true preserves pattern", func(t *testing.T) {
		result := expandWildcardFields(map[string][]ParsedRule{
			"users.*.name": {{Name: "required"}},
		}, []string{"title"}, true)

		assert.Len(t, result, 1)
		assert.Equal(t, "required", result["users.*.name"][0].Name)
	})

	t.Run("keepUnmatched false drops pattern", func(t *testing.T) {
		result := expandWildcardFields(map[string][]ParsedRule{
			"users.*.name": {{Name: "trim"}},
		}, []string{"title"}, false)
