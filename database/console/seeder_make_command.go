...
func (receiver *SeederMakeCommand) Handle(ctx console.Context) error {
	...
	if name == "" {
		var err error
		name, err = ctx.Ask("Enter the seeder name", console.AskOption{
			Validate: func(s string) error {
				if s == "" {
					return errors.New("the seeder name cannot be empty")
				}
	...
	if !force && file.Exists(path) {
		color.Red().Println("The seeder already exists. Use the --force flag to overwrite")
		return nil
	}
