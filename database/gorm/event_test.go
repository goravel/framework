...
var testEventModel = TestEventModel{
	Model: Model{
		...
		Timestamps: Timestamps{
			CreatedAt: carbon.NewLayoutType[carbon.DateTime](carbon.FromStdTime(testNow)),
		},
	},
	Name:     "name",
	Avatar:   "avatar",
...
func (s *EventTestSuite) SetupTest() {
	s.events = []*Event{
		NewEvent(testQuery, &testEventModel, map[string]any{"i_d": 1, "created_at": carbon.NewLayoutType[carbon.DateTime](carbon.FromStdTime(testNow)), "updated_at": carbon.NewLayoutType[carbon.DateTime](carbon.FromStdTime(testNow)), "avatar": "avatar1", "is_admin": false, "manage": 1, "admin_at": time.Now(), "manage_at": testNow}),
		NewEvent(testQuery, &testEventModel, map[string]any{"ID": 1, "CreatedAt": carbon.NewLayoutType[carbon.DateTime](carbon.FromStdTime(testNow)), "UpdatedAt": carbon.NewLayoutType[carbon.DateTime](carbon.FromStdTime(testNow)), "Avatar": "avatar1", "IsAdmin": false, "IsManage": 1, "AdminAt": time.Now(), "ManageAt": testNow}),
		NewEvent(testQuery, &testEventModel, TestEventModel{
			Model: Model{
				ID: 1,
				Timestamps: Timestamps{
					CreatedAt: carbon.NewLayoutType[carbon.DateTime](carbon.FromStdTime(testNow)),
					UpdatedAt: carbon.NewLayoutType[carbon.DateTime](carbon.FromStdTime(testNow)),
				},
			},
...
func (s *EventTestSuite) TestGetAttribute() {
	...
	events := []*Event{
		NewEvent(testQuery, &testEventModel, map[string]any{
			"ID":        2,
			"CreatedAt": carbon.NewLayoutType[carbon.DateTime](now),
			"Avatar":    "avatar1",
		}),
		NewEvent(testQuery, &testEventModel, TestEventModel{
			Model: Model{
	...
	for _, event := range events {
		...
		s.Equal(carbon.NewLayoutType[carbon.DateTime](now), event.GetAttribute("CreatedAt"))
...
func (s *EventTestSuite) TestGetOriginal() {
	...
	s.Equal(carbon.NewLayoutType[carbon.DateTime](carbon.FromStdTime(testNow)), event.GetOriginal("CreatedAt"))
