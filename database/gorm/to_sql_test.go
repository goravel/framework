...
func (s *ToSqlTestSuite) TestCreate() {
	...
	s.Equal("INSERT INTO `users` (`created_at`,`updated_at`,`deleted_at`,`name`,`avatar`) VALUES (?,?,?,?,?)", toSql.Create(&user))

	toSql = NewToSql(s.query.Model(User{}).Where("id", 1).(*QueryImpl), true)
	s.Contains(toSql.Create(&user), "INSERT INTO `users` (`created_at`,`updated_at`,`deleted_at`,`name`,`avatar`) VALUES (")
	s.Contains(toSql.Create(&user), ",NULL,'to_sql_create','')")
...
func (s *ToSqlTestSuite) TestSave() {
	toSql := NewToSql(s.query.Where("id", 1).(*QueryImpl), false)
	s.Equal("INSERT INTO `users` (`created_at`,`updated_at`,`deleted_at`,`name`,`avatar`) VALUES (?,?,?,?,?)", toSql.Save(&User{}))

	toSql = NewToSql(s.query.Where("id", 1).(*QueryImpl), true)
	sql := toSql.Save(&User{})
	s.Contains(sql, "INSERT INTO `users` (`created_at`,`updated_at`,`deleted_at`,`name`,`avatar`) VALUES (")
	s.Contains(sql, ",NULL,'','')")
}
