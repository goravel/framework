...
func Get(config config.Config) (mailcontract.Template, error) {
	driver := config.GetString("mail.template.driver", "default")

	if cached, ok := engines.Load(driver); ok {
		return cached.(mailcontract.Template), nil
	}
	...
	return actual.(mailcontract.Template), nil
}
...
func createEngine(config config.Config, driver string) (mailcontract.Template, error) {
	...
	default:
		...
		case mailcontract.Template:
			return v, nil
		case func() (mailcontract.Template, error):
			engine, err := v()
			if err != nil {
