...
func TestMailMakeCommand(t *testing.T) {
	...
	mockContext.On("Ask", "Enter the mail name", mock.Anything).Return("", errors.New("the mail name cannot be empty")).Once()
	assert.Contains(t, color.CaptureOutput(func(w io.Writer) {
		assert.Nil(t, modelMakeCommand.Handle(mockContext))
	}), "the mail name cannot be empty")
	assert.False(t, file.Exists("app/mails/new_user.go"))

	mockContext.On("Argument", 0).Return("NewUser").Once()
	...
	assert.Contains(t, color.CaptureOutput(func(w io.Writer) {
		assert.Nil(t, modelMakeCommand.Handle(mockContext))
	}), "the mail already exists. Use the --force or -f flag to overwrite")

	mockContext.On("Argument", 0).Return("User/VerifyEmail").Once()
	mockContext.On("OptionBool", "force").Return(false).Once()
	assert.Nil(t, modelMakeCommand.Handle(mockContext))
