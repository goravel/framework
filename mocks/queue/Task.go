...
func (_m *Task) Delay(time uint) queue.Task {
	ret := _m.Called(time)
	...
	var r0 queue.Task
	if rf, ok := ret.Get(0).(func(uint) queue.Task); ok {
		r0 = rf(time)
	} else {
		if ret.Get(0) != nil {
...
func (_c *Task_Delay_Call) Run(run func(time uint)) *Task_Delay_Call {
...
func (_c *Task_Delay_Call) RunAndReturn(run func(uint) queue.Task) *Task_Delay_Call {
	_c.Call.Return(run)
	return _c
}
