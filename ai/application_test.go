...
func TestApplication_Agent_FailoverMatchesConfiguredSubstring(t *testing.T) {
	primaryErr := aiTestError("maximum context length exceeded")
	...
	config := contractsai.Config{
		...
		Providers: map[string]contractsai.ProviderConfig{
			"primary": {
				Via: primaryProvider,
				Failover: map[contractsai.FailoverReason][]string{
...
func TestApplication_Agent_FailoverMatchesConfiguredRegex(t *testing.T) {
	primaryErr := aiTestError("the MODEL is overloaded")
	primaryProvider := &applicationPromptProviderStub{name: "primary", err: primaryErr}
	backupResponse := &stubResponse{text: "backup response"}
	backupProvider := &applicationPromptProviderStub{name: "backup", response: backupResponse}
	config := contractsai.Config{
...
func TestApplication_Agent_InvalidFailoverRegex(t *testing.T) {
	provider := &applicationPromptProviderStub{name: "primary"}
	config := contractsai.Config{
		Default: "primary",
		Providers: map[string]contractsai.ProviderConfig{
			"primary": {
	...
	_, err := NewApplication(context.Background(), config).Agent(mocksai.NewAgent(t))
...
func TestApplication_MediaFailoverMatchesConfiguredRule(t *testing.T) {
	failoverRules := map[contractsai.FailoverReason][]string{
		"context_length_exceeded": {"context length"},
	}
	...
	tests := []struct {
	...
	}{
		{
			name: "audio",
			run: func(t *testing.T) {
				primaryProvider := &applicationAudioProviderStub{err: aiTestError("maximum context length exceeded")}
				backupResponse := &applicationAudioResponseStub{}
				backupProvider := &applicationAudioProviderStub{response: backupResponse}
				app := NewApplication(context.Background(), contractsai.Config{
					Default: "primary",
					Providers: map[string]contractsai.ProviderConfig{
						"primary": {Via: primaryProvider, Failover: failoverRules},
						"backup":  {Via: backupProvider},
					},
				})
		...
		},
		{
			name: "image",
			run: func(t *testing.T) {
				primaryProvider := &applicationImageProviderStub{err: aiTestError("maximum context length exceeded")}
				backupResponse := &applicationImageResponseStub{}
				backupProvider := &applicationImageProviderStub{response: backupResponse}
				app := NewApplication(context.Background(), contractsai.Config{
					Default: "primary",
					Providers: map[string]contractsai.ProviderConfig{
						"primary": {Via: primaryProvider, Failover: failoverRules},
				...
				response, err := app.image(context.Background(), contractsai.ImagePrompt{Prompt: "draw a cat"}, WithProvider("primary", "backup"))

				require.NoError(t, err)
				assert.Equal(t, backupResponse, response)
				assert.Equal(t, "draw a cat", primaryProvider.prompt.Prompt)
				assert.Equal(t, "draw a cat", backupProvider.prompt.Prompt)
			},
		},
		{
			name: "transcription",
			run: func(t *testing.T) {
				file := mocksai.NewStorableFile(t)
				primaryProvider := &applicationTranscriptionProviderStub{err: aiTestError("maximum context length exceeded")}
				backupResponse := &applicationTranscriptionResponseStub{}
				backupProvider := &applicationTranscriptionProviderStub{response: backupResponse}
				app := NewApplication(context.Background(), contractsai.Config{
					Default: "primary",
					Providers: map[string]contractsai.ProviderConfig{
						"primary": {Via: primaryProvider, Failover: failoverRules},
				...
				response, err := app.transcription(context.Background(), contractsai.TranscriptionPrompt{File: file}, WithProvider("primary", "backup"))

				require.NoError(t, err)
				assert.Equal(t, backupResponse, response)
				assert.Equal(t, file, primaryProvider.prompt.File)
				assert.Equal(t, file, backupProvider.prompt.File)
			},
		},
	}
