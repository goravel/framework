	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func() {
				...
				log, err = NewApplication(context.Background(), nil, mockConfig, j, nil)
...
func TestWriter_WithContext(t *testing.T) {
	...
	mockConfig.EXPECT().GetString("app.env").Return("test").Twice()
	// logging.context.exclude is read per handler per log write (single + daily)
...
func TestWriterWithCustomLogger(t *testing.T) {
	...
	logger, err := NewApplication(context.Background(), nil, mockConfig, json.New(), nil)
	assert.Nil(t, err)
...
func (h *CustomHandler) Handle(entry contractslog.Entry) error {
	...
	if filename != "" {
		...
		if len(code) > 0 {
			_, _ = fmt.Fprintf(&builder, "custom_code: %v\n", code)
		}

		if user != nil {
			_, _ = fmt.Fprintf(&builder, "custom_user: %v\n", user)
		}
