...
func Test_getWhichFilesToUpload_onlyUnknownTokens(t *testing.T) {
	// Workspace with all artifacts present
	wd, err := os.Getwd()
	require.NoError(t, err)
	dir := t.TempDir()
	require.NoError(t, os.Chdir(dir))
	...
	mc := &mocksconsole.Context{}
	mc.EXPECT().Option("only").Return("foo,bar").Once()
	hasMain, hasEnv, hasPub, hasStor, hasRes := getWhichFilesToUpload(mc, "myapp", ".env.production")
...
func Test_uploadFilesCommand_EnvOnly(t *testing.T) {
	cmd := uploadFilesCommand("myapp", "203.0.113.10", "22", "ubuntu", "~/.ssh/id", ".env.production", false, true, false, false, false)
	require.NotNil(t, cmd)
	if runtime.GOOS == "windows" {
		t.Skip("Skipping script content assertions on Windows shell")
	}
	script := cmd.Args[2]
	// No binary/public/storage/resources scp
	assert.NotContains(t, script, "/main.new")
	assert.NotContains(t, script, " -r \"public\"")
	assert.NotContains(t, script, " -r \"storage\"")
	assert.NotContains(t, script, " -r \"resources\"")
	// Contains env atomic move
	assert.Contains(t, script, ".env.new")
	assert.Contains(t, script, "/.env'")
}

func Test_setupServerCommand_FirewallOrder_OpenSSHBeforeEnable(t *testing.T) {
	cmd := setupServerCommand("myapp", "203.0.113.10", "9000", "22", "ubuntu", "~/.ssh/id", "example.com", true, true)
	require.NotNil(t, cmd)
	if runtime.GOOS == "windows" {
		t.Skip("Skipping script content assertions on Windows shell")
	...
	script := cmd.Args[2]
	idxSSH := strings.Index(script, "ufw allow OpenSSH")
	idxEnable := strings.LastIndex(script, "ufw --force enable")
	assert.True(t, idxSSH != -1 && idxEnable != -1 && idxSSH < idxEnable, "OpenSSH must be allowed before enabling ufw")
}

func Test_getAllOptions_ExpandsTildeAndBooleans(t *testing.T) {
	// Validate ~ expansion for key path and boolean parsing
	...
	cfg.EXPECT().Env("DEPLOY_DOMAIN").Return("example.com").Once()
	cfg.EXPECT().Env("DEPLOY_PROD_ENV_FILE_PATH").Return(".env.production").Once()
	cfg.EXPECT().Env("DEPLOY_STATIC").Return("true").Once()
	cfg.EXPECT().Env("DEPLOY_REVERSE_PROXY_ENABLED").Return("true").Once()
	cfg.EXPECT().Env("DEPLOY_REVERSE_PROXY_TLS_ENABLED").Return("true").Once()

	appName, _, _, _, _, sshKeyPath, _, _, domain, _, staticEnv, reverseProxyEnabled, reverseProxyTLSEnabled := getAllOptions(ctx, cfg)
	assert.Equal(t, "myapp", appName)
	home, _ := os.UserHomeDir()
	assert.True(t, strings.HasPrefix(sshKeyPath, home+string(os.PathSeparator)))
	assert.Equal(t, "example.com", domain)
	assert.True(t, staticEnv)
	assert.True(t, reverseProxyEnabled)
