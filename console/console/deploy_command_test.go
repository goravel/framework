...
func Test_Handle_Rollback_ShortCircuit(t *testing.T) {
	...
	cfg.EXPECT().GetString("DEPLOY_SSH_IP").Return("203.0.113.10").Once()
	...
	cfg.EXPECT().GetString("DEPLOY_OS").Return("linux").Once()
	cfg.EXPECT().GetString("DEPLOY_ARCH").Return("amd64").Once()
	cfg.EXPECT().GetString("DEPLOY_DOMAIN").Return("").Once()
	cfg.EXPECT().GetString("DEPLOY_PROD_ENV_FILE_PATH").Return(".env.production").Once()
	cfg.EXPECT().GetBool("DEPLOY_STATIC").Return(false).Once()
	cfg.EXPECT().GetBool("DEPLOY_REVERSE_PROXY_ENABLED").Return(false).Once()
	cfg.EXPECT().GetBool("DEPLOY_REVERSE_PROXY_TLS_ENABLED").Return(false).Once()
