...
func (s *WorkerTestSuite) Test_run() {
	...
	s.Run("retry job failed, insert failed job", func() {
		...
		s.mockJson.EXPECT().MarshalString(utils.Task{
			...
			UUID: "test",
		}).Return("{\"signature\":\"test_job_err\",\"args\":[{\"type\":\"string\",\"value\":\"test\"}],\"delay\":null,\"uuid\":\"test\",\"chain\":[]}", nil).Twice()
		...
		s.mockConfig.EXPECT().FailedTable().Return("failed_jobs").Twice()
		s.mockDB.EXPECT().Connection("mysql").Return(s.mockDB).Twice()
		mockQuery := mocksdb.NewQuery(s.T())
		s.mockDB.EXPECT().Table("failed_jobs").Return(mockQuery).Twice()
		mockQuery.EXPECT().Insert(&models.FailedJob{
			UUID:       errorTaskWithChain.UUID,
			Connection: connection,
			Queue:      queue,
